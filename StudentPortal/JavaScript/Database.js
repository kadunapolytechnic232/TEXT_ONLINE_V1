...
setInterval(()=>{
    //ADMIN
    if(localStorage.getItem("VidoeDataURL") != null){

        set(ref(getDatabase(),`VidoeDataURL`),{
            VidoeDataURL:localStorage.getItem("VidoeDataURL")
        })
    }

    //USER
    onValue(ref(db,"VidoeDataURL"),(snapshot)=>{
        VidoeDataURL=[]
        VidoeDataURL.push(snapshot.val())
    })

        
    //SET IMG
    if(VidoeDataURL.length != 0 && VidoeDataURL[0].VidoeDataURL != ""){
        document.querySelector('.Admin_Img').src=VidoeDataURL[0].VidoeDataURL;
    }
    ...
    if(Questions.length != 0){
        ...
        if(localStorage.getItem("EXAM_FLOW_NO2") === null || `${localStorage.getItem("EXAM_FLOW_NO2")}`.split(",").length != Questions[0].split("(/)").length){
            ...
            for(let i=0;i<=`${localStorage.getItem("EXAM_FLOW_NO")}`.split(",").length-1;i++){
                ...
                if(i === 0){
                    localStorage.setItem("EXAM_FLOW_NO2",`${localStorage.getItem("EXAM_FLOW_NO2")}`+','+`${`${localStorage.getItem("EXAM_FLOW_NO")}`.split(",")[0]}`)
                }else if(`${i/2}`.includes('.') === true){
                    localStorage.setItem("EXAM_FLOW_NO2",`${localStorage.getItem("EXAM_FLOW_NO2")}`+','+`${`${localStorage.getItem("EXAM_FLOW_NO")}`.split(",")[i]}`)
                }else if(`${i/2}`.includes('.') === false){
                    localStorage.setItem("EXAM_FLOW_NO2",`${localStorage.getItem("EXAM_FLOW_NO2")}`+','+`${`${localStorage.getItem("EXAM_FLOW_NO")}`.split(",")[`${localStorage.getItem("EXAM_FLOW_NO")}`.split(",").length-i]}`)
                }
...
setInterval(()=>{
    ...
    if(STUDENT_INFORMATION.length != 0 && STUDENT_INFORMATION[0].STUDENT_INFORMATION != "" && `${STUDENT_INFORMATION[0].STUDENT_INFORMATION}`.includes("(//)")){
        if(STUDENT_DATA[0] != STUDENT_INFORMATION[0].STUDENT_INFORMATION.split("(//)")[Number(localStorage.getItem("EXAM_NO"))-1]){
            STUDENT_DATA[0]=STUDENT_INFORMATION[0].STUDENT_INFORMATION.split("(//)")[Number(localStorage.getItem("EXAM_NO"))-1].STUDENT_INFORMATION;
        }
    ...
    if(Questions.length != 0){
        if(localStorage.getItem("Confirm_Submit") === "True"){
            ...
            if(STUDENT_INFORMATION.length !=0 && `${STUDENT_DATA[0]}`.includes(`${localStorage.getItem("IR")}`) != true){
                ...
                if(STUDENT_DATA.length != 0 && STUDENT_DATA[0] != undefined){
    ...
    if(document.querySelector('.User_finish') != null){
        ...
        document.querySelector('.User_finish').addEventListener("click",()=>{
            ...
            for(let i=1;i<=Questions[0].split("(/)").length;i++){
                ...
                localStorage.removeItem(`Select${i}_I`)
                localStorage.removeItem(`Select${i}_II`)
                localStorage.removeItem(`Select${i}_III`)
                localStorage.removeItem(`Select${i}_IV`)
...
document.querySelector('.ADMIN_S_B').addEventListener("click",()=>{
    ...
    /**/
        update(ref(getDatabase(),"EXAM_NUMBER"),{
            ...
            //STUDENT_INFORMATION
            if(STUDENT_INFORMATION.length != 0 && SECTIONS_Q[0].SECTIONS_Q != ""){
...
setInterval(()=>{
    ...
    if(Questions.length != 0 && STUDENT_DATA.length != 0 && STUDENT_DATA[0] != undefined){
        if(STUDENT_DATA[0].split("(/)").length-1 != index_VR){
            ...
            for(let i=0; i <= STUDENT_DATA[0].split("(/)").length-1; i++){
                ...
                //
                localStorage.setItem(`ADMIN_VR_DB_HTML${i}`,`${STUDENT_DATA[0].split('(/)')[i].split('(^)')[3]}`)
                VR_HTML+=`
                <P class="DB_SN">${i+1}</P>
            ...
            DB_VSW.forEach((e)=>{
                    e.addEventListener('click',()=>{
                        ...
                        document.querySelector('.VR_D').style.display="block";
                        document.querySelector('.VR_DB').innerHTML=localStorage.getItem(`ADMIN_VR_DB_HTML${dataDashed_Id}`)+'<p class="ADMIN_finish">Back</p>';
