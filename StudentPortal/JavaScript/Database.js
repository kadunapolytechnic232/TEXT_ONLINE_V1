...
// Update EXAM_NUMBER in localStorage and display it
setInterval(() => {
...
// Update PASSWORD in localStorage and display it
setInterval(() => {
...
// Update Questions based on SECTIONS_Q data
setInterval(() => {
    if (SECTIONS_Q.length !== 0 && SECTIONS_Q[0].SECTIONS_Q !== "" && SECTIONS_Q[0].SECTIONS_Q.includes("(//)")) {
        const sectionQuestion = SECTIONS_Q[0].SECTIONS_Q.split("(//)")[Number(localStorage.getItem("EXAM_NO")) - 1];
        if (Questions[0] !== sectionQuestion) {
            Questions[0] = sectionQuestion;
        }
...
// Ensure C_EXAMTIME is set to TRUE in localStorage if it is not already set
if (localStorage.getItem("C_EXAMTIME") === null) {
    localStorage.setItem("C_EXAMTIME", "TRUE");
}

// Manage EXAMTIME based on conditions
setInterval(() => {
    const examTimeStatus = localStorage.getItem("C_EXAMTIME");
    const hour = localStorage.getItem("Hr");
    const minute = localStorage.getItem("Mt");
    const second = localStorage.getItem("Sd");

    // Handle time and submission when exam time is finished
    if (hour === "00" && minute === "00" && second === "00" && examTimeStatus === "FALSE") {
    ...
    // Handle setting the exam time if C_EXAMTIME is TRUE
    else if (hour === "00" && minute === "00" && second === "00" && examTimeStatus === "TRUE") {
        if (EXAMTIME.length !== 0 && EXAMTIME[0].EXAMTIME !== "" && EXAMTIME[0].EXAMTIME.includes("(/)")) {
            const examTimeParts = EXAMTIME[0].EXAMTIME.split("(/)")[Number(localStorage.getItem("EXAM_NO")) - 1].split("(^)");

            localStorage.setItem("Hr", examTimeParts[0]);
            localStorage.setItem("Mt", examTimeParts[1]);
            localStorage.setItem("Sd", examTimeParts[2]);
            localStorage.setItem("C_EXAMTIME", "FALSE");
        } else if (EXAMTIME.length !== 0 && EXAMTIME[0].EXAMTIME !== "") {
            const examTimeParts = EXAMTIME[0].EXAMTIME.split("(/)")[0].split("(^)");
